...
function Grafica({ filas }: { filas: FilaWeb[] }) {
  ...
  return <><svg className="an-chart-svg" viewBox="0 0 700 230" role="img" aria-label="Vistas de página por día. Consulta la tabla de datos debajo de la gráfica.">
    ...
    {puntoActivo ? <g className="an-tooltip" pointerEvents="none"><rect x={Math.max(8, Math.min(548, puntoActivo.x - 68))} y={Math.max(8, puntoActivo.y - 47)} width="136" height="31" rx="5" /><text x={Math.max(76, Math.min(616, puntoActivo.x))} y={Math.max(28, puntoActivo.y - 27)} textAnchor="middle">{FECHA.format(new Date(puntoActivo.clave))} · {NUMERO.format(puntoActivo.cantidad)} visitas</text></g> : null}
  </svg></>;
}
